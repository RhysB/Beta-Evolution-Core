...
public class EvolutionPlayerListener extends PlayerListener {
    ...
    public void onPlayerJoin(PlayerJoinEvent event) {
        ...
        Bukkit.getServer().getScheduler().scheduleAsyncDelayedTask(plugin, new Runnable() {
            ...
            public void run() {
                try {
                    String url = "https://auth.johnymuffin.com/serverAuth.php?method=1&username=" + URLEncoder.encode(event.getPlayer().getName(), "UTF-8") + "&userip=" + URLEncoder.encode(event.getPlayer().getAddress().getAddress().getHostAddress(), "UTF-8");
